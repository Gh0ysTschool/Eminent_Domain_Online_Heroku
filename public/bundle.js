var app = (function () {
	...
	function create_if_block_15(component, ctx) {
		var div9, div0, text0, div1, text1, div2, text2, text3_value = (ctx.game.displayinfo.center_or_planets) ? "planets" : "center row", text3, text4, text5, text6, div3, text7, text8, text9, div4, text10_value = ctx.game.messagetoplayer[ctx.game.messagetoplayer.length-1], text10, text11, div8, div5, text12, text13_value = ctx.player.deck.length, text13, text14, div6, text15, div7, text16, text17_value = ctx.player.discard.length, text17;
	...
	function create_each_block_12(component, ctx) {
		var div3, div0, text0, text1_value = ctx.p.starfighters.small, text1, text2, div1, text3, text4_value = ctx.p.influence.length, text4, text5, div2, text6_value = ctx.p.name, text6, div3_class_value;
	        ...
	        },
	        {
	          Logistics: () => {
	            if (
	              ...
	              let game = app$1.get().game;
	              if (
	                app$1.get().game.choices[0].name == "Action Phase then Role Phase"
	              ) {
	                game.players[
	                  app$1.get().game.acting_player_index
	                ].actionrolesequence = "ar";
	              } else if (
	                app$1.get().game.choices[0].name == "Role Phase then Action Phase"
	              ) {
	                game.players[
	                  app$1.get().game.acting_player_index
	        ...
	        //      -> improved_trade
	        {
	          "Trading your Stocks and Bonds": () => {
	            if (
	              ...
	              let game = app$1.get().game;
	              game.players[app$1.get().game.acting_player_index].influence.push(
	                game.influence.pop()
	              );
	              app$1.set({ game: game });
	              app$1.phasefinishfunction(true);
	        ...
	        },
	        {
	          "Specializaing in your Seleted Resource": () => {
	            if (
	              ...
	              let game = app$1.get().game;
	              game.players[app$1.get().game.acting_player_index].specialization =
	                game.choices[0].name;
	              app$1.phasefinishfunction(true);
	            }
	          }
	          ...
	          "Discarding your Selected Cards": () => {
	            if (app$1.get().game.choices[0].name == "Skip") {
	              ...
	              for (let i in game.choices) {
	                ...
	                game.players[app$1.get().game.acting_player_index].discard.push(
	                  game.choices[i]
	                );
	              }
	              app$1.set({ game: game });
	      ...
	      cleanup: [
	        {
	          "Drawing up to your Hand Size": () => {
	            ...
	            for (let index in game.players[app$1.get().game.acting_player_index]
	              .settled_planets) {
	              handsize +=
	                game.players[app$1.get().game.acting_player_index]
